  ...
  <div class="xggWrap">
    ...
    <div class="xggMain bg-white pb-2 border-left border-bottom">
      ...
      <div class="container-fluid px-4">
        <h3 class="border-bottom pb-2 mb-5 text-secondary">{{items[items.length-1].text}} <a href="product.html" class="btn btn-success float-right">返回列表</a></h3>
        <div class="py-3 Xggfz14">
          <b-form @submit.prevent="onproduct">
            <b-form-row class="mb-2">
              <div class="col-2 text-right py-1">分类</div>
              <div class="col-4"><b-form-select v-model="product.group_id" :options="groupOps" /></b-form-input></div>
            </b-form-row>
            <b-form-row class="mb-2">
              <div class="col-2 text-right py-1">标题</div>
              <div class="col-4"><b-form-input v-model="product.title" type="text"/></b-form-input></div>
            </b-form-row>
            <b-form-row class="mb-2">
              <div class="col-2 text-right py-1">页面内容</div>
              <div class="col-6"><wangeditor ref='weditor' :inittxt='product.content'></wangeditor></div>
            </b-form-row>
            <b-form-row class="mb-2">
            ...
            <b-form-row class="mb-2">
              ...
              <div class="col-4"><b-form-input v-model="product.keywords" type="text"></b-form-input></div>
            </b-form-row>
            <b-form-row class="mb-2">
              <div class="col-2 text-right py-1">描述</div>
              <div class="col-4"><b-form-textarea v-model="product.description":rows="3" :max-rows="5"></b-form-textarea></div>
            </b-form-row>
            <b-form-row class="mb-2">
              <div class="col-2 text-right py-1">排序</div>
              <div class="col-4"><b-form-input v-model="product.sort" type="text"></b-form-input></div>
            </b-form-row>
            <b-form-row>
...
export default {
  ...
  data(){
    return {
      menucurr:'产品列表',
      items: [{text: '网站管理中心',active: true},{text: '编辑产品',active: true}],
  ...
  mounted(){
    ...
    this.$axios.get(_API+"product/prepare?id="+id)
    .then((res)=>{
  ...
  methods:{
    ...
    onproduct(){
      this.product.content=this.$refs.weditor.getContent();//获取富文本编辑器内容
      // 表单本地验证
      if(this.product.title==''){
        ...
        for(let x in this.product){
          if(x!='image'||x!='image64'){formData.append(x, this.product[x])}
        }
        if(this.product.hasOwnProperty('imagefile')){formData.append('file', this.product.imagefile)}
        let config = {headers: {'Content-Type': 'multipart/form-data'}};

        this.$axios.post(_API+"product/edit?id="+id,formData, config)
        .then((res)=>{
          if(res.data.errcode==0){
            that.timer(3,'编辑产品成功');
          }else if(res.data.errcode==401){
