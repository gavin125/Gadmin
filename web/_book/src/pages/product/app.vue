  ...
  <div class="xggWrap">
    ...
    <div class="xggMain bg-white pb-2 border-left border-bottom">
      ...
      <div class="container-fluid px-4">
        <h3 class="border-bottom pb-2 mb-5 text-secondary">{{items[items.length-1].text}} <a href="product_edit.html" class="btn btn-success float-right px-4">新增</a></h3>
...
export default {
  ...
  mounted () {
    ...
    this.$axios.get(_API+"product"+this.baseurl+page)
    .then((res)=>{
  ...
  methods:{
    ...
    onExecute:function(){
      ...
      if(this.execute.action=='1' && arr.length>0){
        this.$axios.get(_API+'product/execute?action='+this.execute.action+'&checked='+JSON.stringify(arr))
        .then((res)=>{
          if(res.data.errcode==0){
      ...
      if(this.execute.action=='2' && arr.length>0){
        this.$axios.get(_API+'product/execute?group='+this.execute.group+'&action='+this.execute.action+'&checked='+JSON.stringify(arr))
        .then((res)=>{
          if(res.data.errcode==0){
            window.location.href=window.location.href;
          }else if(res.data.errcode==401){
