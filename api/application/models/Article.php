...
class ArticleModel {
  ...
  public function getarticles($page,$size,$group_id='',$text='') {
    if(!$this->_isadmin()){return false;}
    $SQLSTR='TRUE';
    if($group_id!=''){ $SQLSTR.=' AND a.group_id='.$group_id;}
    if($text!=''){ $SQLSTR.=' AND a.title LIKE "%'.$text.'%"';}
    
    $sth=$this->_pdo->query('SELECT a.id,a.src,a.title,a.add_time,a.sort,b.name FROM xgg_article AS a LEFT JOIN xgg_article_group AS b ON a.group_id=b.id WHERE '.$SQLSTR.' LIMIT '.($page-1)*$size.','.$size);
  ...
  public function getarticle($id){
    ...
    $sth=$this->_pdo->prepare('SELECT group_id,title,content,src AS image,keywords,description,sort FROM xgg_article WHERE id=?');
  ...
  public function gettotel($group_id='',$text='') {
    ...
    $sth=$this->_pdo->query('SELECT id FROM xgg_article WHERE '.$SQLSTR);
    return $sth->rowCount();
  }
  ...
  public function execute($action,$group,$checked){
    ...
    if($action=='2'){
      if($group==''){$group=null;}
      foreach ($checked as $k => $v) {
        $sth=$this->_pdo->prepare('UPDATE xgg_article SET group_id=?  WHERE id=?');
  ...
  public function add($group_id,$title,$content,$imagefile,$keywords,$description,$sort) {
    ...
    $sth=$this->_pdo->prepare('INSERT INTO xgg_article (group_id,title,content,keywords,description,add_time,sort) VALUES (?,?,?,?,?,?,?)');
    $sth->execute(array($group_id,$title,$content,$keywords,$description,time(),$sort));
    $id=$this->_pdo->lastInsertId();
    if($imagefile){
      $sth=$this->_pdo->prepare('UPDATE xgg_article SET src=? WHERE id=?');
  ...
  public function update($group_id,$title,$content,$imagefile,$keywords,$description,$sort,$id) {
    ...
    if($group_id==''){$group_id=null;}
    $sth=$this->_pdo->prepare('UPDATE xgg_article SET group_id=?,title=?,content=?,keywords=?,description=?,sort=? WHERE id=?');
